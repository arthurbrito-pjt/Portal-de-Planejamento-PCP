    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => onNavigateToSubview('prontidao-demanda')} className="text-[11px] text-blue-700 hover:underline font-medium">
            Ver todos
          </button>
        </div>
        ...
        {topReadySlitters.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {topReadySlitters.map(item => {
              ...
              return (
                <div key={item.id} className="p-3 rounded-lg border border-slate-200 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold font-mono text-blue-800">{item.codigoSlitter}</span>
                    <MetricsBadge type="familia" value={item.mainProduct.familia} size="sm" />
                  </div>
                  <div className="text-[11px] text-slate-500 line-clamp-1">{item.nomeSlitter}</div>
                  <div className="flex items-center justify-between text-[11px] font-mono text-slate-600">
                    <span>{item.totalDemandaT}t demanda</span>
                    <span className="text-emerald-700">{item.estimatedYieldPercent}% rend.</span>
                  </div>
                  <button
